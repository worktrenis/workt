...
export class NetEarningsCalculator {
  ...
  /**
   * 🆕 NUOVO: Metodi che utilizzano il calcolatore reale
   */
  
  // Calcolo basato esclusivamente sui dati reali
  calculateRealPayslipNet(grossAmount) {
    if (realPayslipCalculator) {
      return realPayslipCalculator.calculateRealNet(grossAmount);
    }
    return this.calculateQuickNet(grossAmount);
  }

  // Validazione accuratezza del calcolatore
  validateCalculatorAccuracy() {
    if (realPayslipCalculator) {
      return realPayslipCalculator.validateAccuracy();
    }
    return { accuracy: 'non disponibile', reason: 'Calcolatore reale non inizializzato' };
  }

  // Riepilogo buste paga utilizzate
  getPayslipDataSummary() {
    if (realPayslipCalculator) {
      return realPayslipCalculator.getPayslipSummary();
    }
    return { error: 'Dati buste paga non disponibili' };
  }
}
...
// 🆕 FUNZIONI BASATE SU DATI REALI
export const calculateRealNet = (gross) => netCalculator.calculateRealPayslipNet(gross);
export const validateNetCalculation = () => netCalculator.validateCalculatorAccuracy();
export const getPayslipSummary = () => netCalculator.getPayslipDataSummary();
